...
```
X-Agent-API-Key: agk_YOUR_AGENT_KEY_HERE
```
...
curl -X POST 'https://<域名>/api/open/agent/dsh/tasks' \
  ...
  -H 'X-Agent-API-Key: agk_YOUR_AGENT_KEY_HERE' \
...
视频能力 = 口播短视频（文字 → TTS 人声 + 字幕 + 背景 + BGM 渲染 MP4），非文生视频。完整链路 = `POST /videos` 创建 → `POST /videos/analyze-article`（可选，从文章生成口播稿）→ `POST /videos/:id/submit` 出片 → 轮询状态 → `GET /videos/:id/play-url` 取播放签名 URL 交付用户。
