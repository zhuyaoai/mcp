...
| `video_publish` | 发布视频到抖音（独立授权；`accountId` 可不传，系统自动选公司第一个可用抖音账号；`publishMode`=publish / draft） |

> 视频能力 = 口播短视频（文字 → TTS 人声 + 字幕 + 背景 + BGM 渲染 MP4），非文生视频。完整链路 = video_create →（可选 video_analyze_article）→ video_submit_build → video_status 轮询 →（可选 video_publish）。
...
4. **video_flow 视频制作全流程**：video_templates（查公司模板，确认默认模板 `isCompanyDefault=true`）→ video_create（不传 templateId 自动套默认模板，或传 templateId 固定风格）→（可选 video_analyze_article 从文章生成口播稿）→ video_submit_build → video_status 轮询至 done →（可选 video_publish 发布抖音）
...
- knowledge_fill、article_create、article_update、article_publish、article_generate、publish_trigger
...
- video_create、video_analyze_article、video_submit_build、video_publish
